...
public class Conexion {
	...
	//Si no ponemos lo de serverTimezone habría que cambiar en el servidor mysql la variable global
	//time_zone -> show global variables like 'time_zone'; o select @@global.time_zone; devuelve SYSTEM y 
	//debería coger la configuración válida de nuestra zona horaria del sistema operativo, pero no lo hace
	...
	//lo mejor es dejar a mysql como está y añadir al URL de la base de datos ?serverTimezone=UTC (que debería encajar
	//con lo que haya en nuestro sistema.
	private final static String URL = "jdbc:mysql://localhost:3306/"+Conexion.BASE+"?serverTimezone=UTC";
    
    public static Connection getConexionBdFerreteria() throws Exception {
		
		try{
		   //Class.forName("com.mysql.cj.jdbc.Driver");
		   return Conexion.conexion=DriverManager.getConnection(Conexion.URL, Conexion.USER, Conexion.PASSWORD); //Abrir conexión TCP/IP
		}
    ...
    public static void close()  {
        if (Conexion.conexion!=null) {
            ...
            } catch (SQLException ex)  {  }// No hacemos nada
        }
    }
}
